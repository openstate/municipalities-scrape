...
gemeentes$besluitvorming[79] <- str_replace(gemeentes$Website[79], "www.", "besluitvorming.")
...
# if (is.na(gemeentes$ibabs[202]) & is.na(gemeentes$notubiz[i]) & is.na(gemeentes$url_regular[i])) {
#   if (!is.na(gemeentes$Website[i])) {
#     if (tryCatch({
#       LinkExtractor(gemeentes$Website[202])
#       TRUE
#     }, error=function(e) FALSE)) {
#       
#       links_internal <- LinkExtractor(gemeentes$Website[346])$InternalLinks
#       links_external <- LinkExtractor(gemeentes$Website[346])$ExternalLinks
#       links <- c(links_internal, links_external)
#       links_sub <- sub(".*nl", "", links)
...
#                                        raadsleden|Raadsleden|Bestuur|bestuur|inwoners|gemeente", 
...
#                                                            vragen|brieven|actueel", links_sub))]
#       links2 <- list()
#       links_sub2 <- list()
#       links_grepl2 <- list()
#       for (t in seq_along(links_grepl)) {
...
#                                                               vragen|brieven|actueel", links_sub2))]
#       links_grepl2 <- unique(unlist(links_grepl2))
#       
#       links3 <- list()
#       links_sub3 <- list()
...
#                                                               bekendmakingen|whatsapp|tel:|mailto:|regelingen|Regelingen|documenten|Documenten|
#                                                               Burgemeester|burgemeester|advies|Advies|Notulen|notulen|Sociale|sociale|
#                                                               vragen|brieven|actueel", links_sub3))]
#       links_grepl3 <- unique(unlist(links_grepl3))
#       
#       links4 <- list()
#       links_sub4 <- list()
...
#                                                               bekendmakingen|whatsapp|tel:|mailto:|regelingen|Regelingen|documenten|Documenten|
#                                                               Burgemeester|burgemeester|advies|Advies|Notulen|notulen|Sociale|sociale|
#                                                               vragen|brieven|actueel", links_sub4))]
#       links_grepl4 <- unique(unlist(links_grepl4))
#       
#       links_merge <- unique(c(links_grepl, links_grepl2, links_grepl3, links_grepl4))
#       links_merge <- links_merge[which(!grepl("Vergaderingen|vergaderingen|verordeningen|whatsapp|RSS|rss|Nieuws|nieuws|
#                                               bekendmakingen|whatsapp|tel:|mailto:|regelingen|Regelingen|documenten|Documenten|
#                                               Burgemeester|burgemeester|advies|Advies|Notulen|notulen|Sociale|sociale|inwoners|Inwoners|vragen|actueel", links_merge))]
#       
#       for (c in seq_along(links_merge)) {
#         if (tryCatch({
#           links_merge[c] %>%
...
#       
#       gemeentes$url_regular[346] <- ifelse(length(samenstelling_gemeenteraad) > 0, samenstelling_gemeenteraad,
#                                          ifelse(length(gemeenteraad_samenstelling) > 0, gemeenteraad_samenstelling, 
#                                                 ifelse(length(samenstelling_raad) > 0, samenstelling_raad,
#                                                        ifelse(length(Samenstelling_gemeenteraad) > 0, Samenstelling_gemeenteraad, 
#                                                               ifelse(length(wie) > 0, wie, NA)))))
#       
#       if (length(links_merge) > 0 & is.na(gemeentes$url_regular[346])) {
#         lengths <- NA
#         for (j in seq_along(links_merge)) {
#           lengths[j] <- length(links_merge[j] %>%
#                                  read_html() %>%
...
#                                  html_attr("href"))
#         }
#         gemeentes$url_regular[346] <- links_merge[which.max(lengths)] 
#       }
#     } else {
#       gemeentes$url_regular[346] <- NA
#     }
#   } else {
#     gemeentes$url_regular[i] <- NA
#   }
...
for (i in seq_along(gemeentes$Gemeente)) {
  ...
  if (length(which(c(!is.na(gemeentes$url_regular[i]),
    ...
    if (length(unique(multiple)) > 1) {
      print(gemeentes$Gemeente[i])
      gemeentes$use_url[i] <- multiple[1]
    } else {
